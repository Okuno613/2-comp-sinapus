...
void calv(double *v,double *u,double *s,double *s_egp,double *A_egp,double *V_s, double *n_s,double *V_d,double *h_d, double *n_d,double *p_d,  int *spike_s,int *spike_d, double *inp, double t,int *spikecnt_s,int *spikecnt_d,int *spikecnt_v,int *count_s,int *count_d,double *THl,FILE *fp1,FILE *fp2,FILE *fp3,FILE *fp4,FILE *fp5,FILE *fp6,FILE *fp7)
{
  ...
  for(int i=0;i<NUM;i++){
    ...
    */
    ...
    if(	0	<t and t<=	100	){	inp[i]=I0* ( (	1.000000 	 +t*	0.000000 	) *  exp( -( (i-(	47.69277	 +t*	0.000169 	))*(i-(	47.69277	 +t*	0.000169 	))) / (2*(	23.4093	 +t*	0.000309 	 )*(	23.4093	 +t*	0.000309 	) ))); }
  	if(	100	<t and t<=	200	){	inp[i]=I0* ( (	0.982939 	 +(t-100)*	-0.000002 	) *  exp( -( (i-(	49.38084	 +(t-100)*	-0.000046 	))*(i-(	49.38084	 +(t-100)*	-0.000046 	))) / (2*(	26.50258	 +(t-100)*	0.000074 	 )*(	26.50258	 +(t-100)*	0.000074 	) ))); }
  	if(	200	<t and t<=	300	){	inp[i]=I0* ( (	0.999545 	 +(t-200)*	0.000002 	) *  exp( -( (i-(	48.91876	 +(t-200)*	-0.000036 	))*(i-(	48.91876	 +(t-200)*	-0.000036 	))) / (2*(	27.2436	 +(t-200)*	-0.000288 	 )*(	27.2436	 +(t-200)*	-0.000288 	) ))); }
  	if(	300	<t and t<=	400	){	inp[i]=I0* ( (	0.981347 	 +(t-300)*	-0.000002 	) *  exp( -( (i-(	48.56338	 +(t-300)*	-0.000218 	))*(i-(	48.56338	 +(t-300)*	-0.000218 	))) / (2*(	24.36694	 +(t-300)*	-0.000133 	 )*(	24.36694	 +(t-300)*	-0.000133 	) ))); }
  	if(	400	<t and t<=	500	){	inp[i]=I0* ( (	0.952229 	 +(t-400)*	-0.000003 	) *  exp( -( (i-(	46.38368	 +(t-400)*	-0.000069 	))*(i-(	46.38368	 +(t-400)*	-0.000069 	))) / (2*(	23.0359	 +(t-400)*	0.000055 	 )*(	23.0359	 +(t-400)*	0.000055 	) ))); }
  	if(	500	<t and t<=	700	){	inp[i]=I0* ( (	0.977025 	 +(t-500)*	0.000002 	) *  exp( -( (i-(	45.69522	 +(t-500)*	-0.004570 	))*(i-(	45.69522	 +(t-500)*	-0.004570 	))) / (2*(	23.58759	 +(t-500)*	-0.002359 	 )*(	23.58759	 +(t-500)*	-0.002359 	) ))); }
  	if(	700	<t and t<=	800	){	inp[i]=I0* ( (	0.924249 	 +(t-700)*	0.000092 	) *  exp( -( (i-(	41.61367	 +(t-700)*	-0.000046 	))*(i-(	41.61367	 +(t-700)*	-0.000046 	))) / (2*(	25.50855	 +(t-700)*	0.000226 	 )*(	25.50855	 +(t-700)*	0.000226 	) ))); }
  	if(	800	<t and t<=	900	){	inp[i]=I0* ( (	0.909463 	 +(t-800)*	-0.000001 	) *  exp( -( (i-(	41.15174	 +(t-800)*	-0.000431 	))*(i-(	41.15174	 +(t-800)*	-0.000431 	))) / (2*(	27.76699	 +(t-800)*	0.000020 	 )*(	27.76699	 +(t-800)*	0.000020 	) ))); }
  	if(	900	<t and t<=	1000	){	inp[i]=I0* ( (	0.869882 	 +(t-900)*	-0.000004 	) *  exp( -( (i-(	36.84341	 +(t-900)*	-0.000099 	))*(i-(	36.84341	 +(t-900)*	-0.000099 	))) / (2*(	27.97097	 +(t-900)*	-0.000134 	 )*(	27.97097	 +(t-900)*	-0.000134 	) ))); }
  	if(	1000	<t and t<=	1100	){	inp[i]=I0* ( (	0.861692 	 +(t-1000)*	-0.000001 	) *  exp( -( (i-(	35.84879	 +(t-1000)*	-0.000234 	))*(i-(	35.84879	 +(t-1000)*	-0.000234 	))) / (2*(	26.62877	 +(t-1000)*	0.000122 	 )*(	26.62877	 +(t-1000)*	0.000122 	) ))); }
  	if(	1100	<t and t<=	1200	){	inp[i]=I0* ( (	0.814604 	 +(t-1100)*	-0.000005 	) *  exp( -( (i-(	33.50837	 +(t-1100)*	-0.000522 	))*(i-(	33.50837	 +(t-1100)*	-0.000522 	))) / (2*(	27.8527	 +(t-1100)*	-0.000935 	 )*(	27.8527	 +(t-1100)*	-0.000935 	) ))); }
  	if(	1200	<t and t<=	1300	){	inp[i]=I0* ( (	1.621702 	 +(t-1200)*	0.000081 	) *  exp( -( (i-(	28.29178	 +(t-1200)*	0.000247 	))*(i-(	28.29178	 +(t-1200)*	0.000247 	))) / (2*(	18.50414	 +(t-1200)*	0.000618 	 )*(	18.50414	 +(t-1200)*	0.000618 	) ))); }
  	if(	1300	<t and t<=	1400	){	inp[i]=I0* ( (	0.763194 	 +(t-1300)*	-0.000086 	) *  exp( -( (i-(	30.76049	 +(t-1300)*	-0.000263 	))*(i-(	30.76049	 +(t-1300)*	-0.000263 	))) / (2*(	24.68098	 +(t-1300)*	0.000300 	 )*(	24.68098	 +(t-1300)*	0.000300 	) ))); }
  	if(	1400	<t and t<=	1500	){	inp[i]=I0* ( (	0.716106 	 +(t-1400)*	-0.000005 	) *  exp( -( (i-(	28.13071	 +(t-1400)*	-0.000092 	))*(i-(	28.13071	 +(t-1400)*	-0.000092 	))) / (2*(	27.68557	 +(t-1400)*	0.000378 	 )*(	27.68557	 +(t-1400)*	0.000378 	) ))); }
  	if(	1500	<t and t<=	1600	){	inp[i]=I0* ( (	0.670837 	 +(t-1500)*	-0.000005 	) *  exp( -( (i-(	27.21307	 +(t-1500)*	-0.000104 	))*(i-(	27.21307	 +(t-1500)*	-0.000104 	))) / (2*(	31.46205	 +(t-1500)*	0.000119 	 )*(	31.46205	 +(t-1500)*	0.000119 	) ))); }
  	if(	1600	<t and t<=	1700	){	inp[i]=I0* ( (	0.624204 	 +(t-1600)*	-0.000005 	) *  exp( -( (i-(	26.16911	 +(t-1600)*	0.000133 	))*(i-(	26.16911	 +(t-1600)*	0.000133 	))) / (2*(	32.65031	 +(t-1600)*	0.000147 	 )*(	32.65031	 +(t-1600)*	0.000147 	) ))); }
  	if(	1700	<t and t<=	1800	){	inp[i]=I0* ( (	0.583258 	 +(t-1700)*	-0.000004 	) *  exp( -( (i-(	27.49416	 +(t-1700)*	-0.000418 	))*(i-(	27.49416	 +(t-1700)*	-0.000418 	))) / (2*(	34.11608	 +(t-1700)*	-0.000331 	 )*(	34.11608	 +(t-1700)*	-0.000331 	) ))); }
  	if(	1800	<t and t<=	1900	){	inp[i]=I0* ( (	1.026388 	 +(t-1800)*	0.000044 	) *  exp( -( (i-(	23.31769	 +(t-1800)*	0.000979 	))*(i-(	23.31769	 +(t-1800)*	0.000979 	))) / (2*(	30.80132	 +(t-1800)*	0.000400 	 )*(	30.80132	 +(t-1800)*	0.000400 	) ))); }
  	if(	1900	<t and t<=	2000	){	inp[i]=I0* ( (	0.572338 	 +(t-1900)*	-0.000045 	) *  exp( -( (i-(	33.11001	 +(t-1900)*	0.000515 	))*(i-(	33.11001	 +(t-1900)*	0.000515 	))) / (2*(	34.79955	 +(t-1900)*	-0.000221 	 )*(	34.79955	 +(t-1900)*	-0.000221 	) ))); }
  	if(	2000	<t and t<=	2100	){	inp[i]=I0* ( (	0.575068 	 +(t-2000)*	0.000000 	) *  exp( -( (i-(	38.26434	 +(t-2000)*	0.000514 	))*(i-(	38.26434	 +(t-2000)*	0.000514 	))) / (2*(	32.58507	 +(t-2000)*	0.000014 	 )*(	32.58507	 +(t-2000)*	0.000014 	) ))); }
  	if(	2100	<t and t<=	2200	){	inp[i]=I0* ( (	0.610100 	 +(t-2100)*	0.000004 	) *  exp( -( (i-(	43.40053	 +(t-2100)*	0.000386 	))*(i-(	43.40053	 +(t-2100)*	0.000386 	))) / (2*(	32.7223	 +(t-2100)*	-0.000418 	 )*(	32.7223	 +(t-2100)*	-0.000418 	) ))); }
  	if(	2200	<t and t<=	2300	){	inp[i]=I0* ( (	0.708371 	 +(t-2200)*	0.000010 	) *  exp( -( (i-(	47.26221	 +(t-2200)*	0.000191 	))*(i-(	47.26221	 +(t-2200)*	0.000191 	))) / (2*(	28.54399	 +(t-2200)*	-0.000114 	 )*(	28.54399	 +(t-2200)*	-0.000114 	) ))); }
  	if(	2300	<t and t<=	2400	){	inp[i]=I0* ( (	0.826206 	 +(t-2300)*	0.000012 	) *  exp( -( (i-(	49.17341	 +(t-2300)*	-0.000148 	))*(i-(	49.17341	 +(t-2300)*	-0.000148 	))) / (2*(	27.40751	 +(t-2300)*	-0.000400 	 )*(	27.40751	 +(t-2300)*	-0.000400 	) ))); }
  	if(	2400	<t and t<=	2500	){	inp[i]=I0* ( (	1.000000 	 +(t-2400)*	0.000017 	) *  exp( -( (i-(	47.69277	 +(t-2400)*	0.000000 	))*(i-(	47.69277	 +(t-2400)*	0.000000 	))) / (2*(	23.4093	 +(t-2400)*	0.000000 	 )*(	23.4093	 +(t-2400)*	0.000000 	) ))); }
