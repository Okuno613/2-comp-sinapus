...
void calv(double *v,double *u,double *s,double *s_egp,double *A_egp,double *V_s, double *n_s,double *V_d,double *h_d, double *n_d,double *p_d,  int *spike_s,int *spike_d, double *inp, double t,int *spikecnt_s,int *spikecnt_d,int *count_s,int *count_d,double *THl,FILE *fp1,FILE *fp2,FILE *fp3,FILE *fp4,FILE *fp5,FILE *fp6,FILE *fp7)
{
  ...
  for(int i=0;i<NUM;i++){
    ...
    */
    if(	0	<t<	100	){	inp[i]=I0* ( (	1	 +t*	0.000000000000 	) *  exp( -( (i-(	68.36212	 +t*	0.000000000000 	))*(i-(	68.36212	 +t*	0.000000000000 	))) / (2*(	33.00741	 +t*	0.000000000000 	 )*(	33.00741	 +t*	0.000000000000 	) ))); }
  	if(	100	<t<	200	){	inp[i]=I0* ( (	0.947677837	 +t*	-0.000005232216 	) *  exp( -( (i-(	70.85719	 +t*	0.000249507000 	))*(i-(	70.85719	 +t*	0.000249507000 	))) / (2*(	44.69124	 +t*	0.001168383000 	 )*(	44.69124	 +t*	0.001168383000 	) ))); }
  	if(	200	<t<	300	){	inp[i]=I0* ( (	0.917695473	 +t*	-0.000002998236 	) *  exp( -( (i-(	70.38419	 +t*	-0.000047300000 	))*(i-(	70.38419	 +t*	-0.000047300000 	))) / (2*(	35.48321	 +t*	-0.000920803000 	 )*(	35.48321	 +t*	-0.000920803000 	) ))); }
  	if(	300	<t<	400	){	inp[i]=I0* ( (	0.865961199	 +t*	-0.000005173427 	) *  exp( -( (i-(	71.683	 +t*	0.000129881000 	))*(i-(	71.683	 +t*	0.000129881000 	))) / (2*(	38.33532	 +t*	0.000285211000 	 )*(	38.33532	 +t*	0.000285211000 	) ))); }
  	if(	400	<t<	500	){	inp[i]=I0* ( (	0.833627278	 +t*	-0.000003233392 	) *  exp( -( (i-(	69.71406	 +t*	-0.000196894000 	))*(i-(	69.71406	 +t*	-0.000196894000 	))) / (2*(	37.96867	 +t*	-0.000036665000 	 )*(	37.96867	 +t*	-0.000036665000 	) ))); }
  	if(	500	<t<	600	){	inp[i]=I0* ( (	0.841269841	 +t*	0.000000764256 	) *  exp( -( (i-(	67.31111	 +t*	-0.000240295000 	))*(i-(	67.31111	 +t*	-0.000240295000 	))) / (2*(	43.19896	 +t*	0.000523029000 	 )*(	43.19896	 +t*	0.000523029000 	) ))); }
  	if(	600	<t<	700	){	inp[i]=I0* ( (	0.815402704	 +t*	-0.000002586714 	) *  exp( -( (i-(	66.60493	 +t*	-0.000070618000 	))*(i-(	66.60493	 +t*	-0.000070618000 	))) / (2*(	33.86756	 +t*	-0.000933140000 	 )*(	33.86756	 +t*	-0.000933140000 	) ))); }
  	if(	700	<t<	800	){	inp[i]=I0* ( (	0.778953557	 +t*	-0.000003644915 	) *  exp( -( (i-(	65.55897	 +t*	-0.000104596000 	))*(i-(	65.55897	 +t*	-0.000104596000 	))) / (2*(	37.83425	 +t*	0.000396669000 	 )*(	37.83425	 +t*	0.000396669000 	) ))); }
  	if(	800	<t<	900	){	inp[i]=I0* ( (	0.753674309	 +t*	-0.000002527925 	) *  exp( -( (i-(	63.88651	 +t*	-0.000167246000 	))*(i-(	63.88651	 +t*	-0.000167246000 	))) / (2*(	40.74721	 +t*	0.000291296000 	 )*(	40.74721	 +t*	0.000291296000 	) ))); }
  	if(	900	<t<	1000	){	inp[i]=I0* ( (	0.735449735	 +t*	-0.000001822457 	) *  exp( -( (i-(	61.96831	 +t*	-0.000191820000 	))*(i-(	61.96831	 +t*	-0.000191820000 	))) / (2*(	35.55546	 +t*	-0.000519175000 	 )*(	35.55546	 +t*	-0.000519175000 	) ))); }
  	if(	1000	<t<	1100	){	inp[i]=I0* ( (	0.733098178	 +t*	-0.000000235156 	) *  exp( -( (i-(	59.54127	 +t*	-0.000242704000 	))*(i-(	59.54127	 +t*	-0.000242704000 	))) / (2*(	37.23015	 +t*	0.000167469000 	 )*(	37.23015	 +t*	0.000167469000 	) ))); }
  	if(	1100	<t<	1200	){	inp[i]=I0* ( (	0.723104056	 +t*	-0.000000999412 	) *  exp( -( (i-(	56.93742	 +t*	-0.000260385000 	))*(i-(	56.93742	 +t*	-0.000260385000 	))) / (2*(	32.77647	 +t*	-0.000445368000 	 )*(	32.77647	 +t*	-0.000445368000 	) ))); }
  	if(	1200	<t<	1300	){	inp[i]=I0* ( (	0.69664903	 +t*	-0.000002645503 	) *  exp( -( (i-(	53.67773	 +t*	-0.000325969000 	))*(i-(	53.67773	 +t*	-0.000325969000 	))) / (2*(	37.25089	 +t*	0.000447442000 	 )*(	37.25089	 +t*	0.000447442000 	) ))); }
  	if(	1300	<t<	1400	){	inp[i]=I0* ( (	0.68547913	 +t*	-0.000001116990 	) *  exp( -( (i-(	54.36619	 +t*	0.000068846000 	))*(i-(	54.36619	 +t*	0.000068846000 	))) / (2*(	39.1534	 +t*	0.000190251000 	 )*(	39.1534	 +t*	0.000190251000 	) ))); }
  	if(	1400	<t<	1500	){	inp[i]=I0* ( (	0.660199882	 +t*	-0.000002527925 	) *  exp( -( (i-(	51.06403	 +t*	-0.000330216000 	))*(i-(	51.06403	 +t*	-0.000330216000 	))) / (2*(	36.35997	 +t*	-0.000279343000 	 )*(	36.35997	 +t*	-0.000279343000 	) ))); }
  	if(	1500	<t<	1600	){	inp[i]=I0* ( (	0.659024103	 +t*	-0.000000117578 	) *  exp( -( (i-(	52.06237	 +t*	0.000099834000 	))*(i-(	52.06237	 +t*	0.000099834000 	))) / (2*(	37.13372	 +t*	0.000077375000 	 )*(	37.13372	 +t*	0.000077375000 	) ))); }
  	if(	1600	<t<	1700	){	inp[i]=I0* ( (	0.639035861	 +t*	-0.000001998824 	) *  exp( -( (i-(	47.84782	 +t*	-0.000421455000 	))*(i-(	47.84782	 +t*	-0.000421455000 	))) / (2*(	40.58015	 +t*	0.000344643000 	 )*(	40.58015	 +t*	0.000344643000 	) ))); }
  	if(	1700	<t<	1800	){	inp[i]=I0* ( (	0.643151088	 +t*	0.000000411523 	) *  exp( -( (i-(	49.97792	 +t*	0.000213010000 	))*(i-(	49.97792	 +t*	0.000213010000 	))) / (2*(	39.83974	 +t*	-0.000074041000 	 )*(	39.83974	 +t*	-0.000074041000 	) ))); }
  	if(	1800	<t<	1900	){	inp[i]=I0* ( (	0.623750735	 +t*	-0.000001940035 	) *  exp( -( (i-(	49.47081	 +t*	-0.000050711000 	))*(i-(	49.47081	 +t*	-0.000050711000 	))) / (2*(	40.62125	 +t*	0.000078151000 	 )*(	40.62125	 +t*	0.000078151000 	) ))); }
  	if(	1900	<t<	2000	){	inp[i]=I0* ( (	0.642563198	 +t*	0.000001881246 	) *  exp( -( (i-(	52.47719	 +t*	0.000300638000 	))*(i-(	52.47719	 +t*	0.000300638000 	))) / (2*(	39.62862	 +t*	-0.000099263000 	 )*(	39.62862	 +t*	-0.000099263000 	) ))); }
  	if(	2000	<t<	2100	){	inp[i]=I0* ( (	0.661375661	 +t*	0.000001881246 	) *  exp( -( (i-(	56.51318	 +t*	0.000403599000 	))*(i-(	56.51318	 +t*	0.000403599000 	))) / (2*(	38.08453	 +t*	-0.000154409000 	 )*(	38.08453	 +t*	-0.000154409000 	) ))); }
  	if(	2100	<t<	2200	){	inp[i]=I0* ( (	0.691358025	 +t*	0.000002998236 	) *  exp( -( (i-(	60.08708	 +t*	0.000357390000 	))*(i-(	60.08708	 +t*	0.000357390000 	))) / (2*(	37.88907	 +t*	-0.000019546000 	 )*(	37.88907	 +t*	-0.000019546000 	) ))); }
  	if(	2200	<t<	2300	){	inp[i]=I0* ( (	0.743680188	 +t*	0.000005232216 	) *  exp( -( (i-(	61.44753	 +t*	0.000136045000 	))*(i-(	61.44753	 +t*	0.000136045000 	))) / (2*(	47.09735	 +t*	0.000920828000 	 )*(	47.09735	 +t*	0.000920828000 	) ))); }
  	if(	2300	<t<	2400	){	inp[i]=I0* ( (	0.855967078	 +t*	0.000011228689 	) *  exp( -( (i-(	67.29316	 +t*	0.000584563000 	))*(i-(	67.29316	 +t*	0.000584563000 	))) / (2*(	39.20149	 +t*	-0.000789586000 	 )*(	39.20149	 +t*	-0.000789586000 	) ))); }
  	if(	2400	<t<	2500	){	inp[i]=I0* ( (	1	 +t*	0.000014403292 	) *  exp( -( (i-(	68.36212	 +t*	0.000106896000 	))*(i-(	68.36212	 +t*	0.000106896000 	))) / (2*(	33.00741	 +t*	-0.000619408000 	 )*(	33.00741	 +t*	-0.000619408000 	) ))); }
