...
void calv(double *v,double *u,double *s,double *s_egp,double *A_egp,double *V_s, double *n_s,double *V_d,double *h_d, double *n_d,double *p_d,  int *spike_s,int *spike_d, double *inp, double t,int *spikecnt_s,int *spikecnt_d,int *spikecnt_v,int *count_s,int *count_d,double *THl,FILE *fp1,FILE *fp2,FILE *fp3,FILE *fp4,FILE *fp5,FILE *fp6,FILE *fp7)
{
  ...
  for(int i=0;i<NUM;i++){
    ...
    */
    if(	0	<t and t<=	100	){	inp[i]=I0* ( (	1.000000 	 +t*	-0.000003 	) *  exp( -( (i-(	96.10438	 +t*	0.000037 	))*(i-(	96.10438	 +t*	0.000037 	))) / (2*(	38.83282	 +t*	0.000475 	 )*(	38.83282	 +t*	0.000475 	) ))); }
  	if(	100	<t and t<=	200	){	inp[i]=I0* ( (	0.967266 	 +(t-100)*	-0.000001 	) *  exp( -( (i-(	96.47931	 +(t-100)*	0.000068 	))*(i-(	96.47931	 +(t-100)*	0.000068 	))) / (2*(	43.57787	 +(t-100)*	-0.000517 	 )*(	43.57787	 +(t-100)*	-0.000517 	) ))); }
  	if(	200	<t and t<=	300	){	inp[i]=I0* ( (	0.955970 	 +(t-200)*	-0.000002 	) *  exp( -( (i-(	97.16075	 +(t-200)*	-0.000073 	))*(i-(	97.16075	 +(t-200)*	-0.000073 	))) / (2*(	38.40746	 +(t-200)*	0.000119 	 )*(	38.40746	 +(t-200)*	0.000119 	) ))); }
  	if(	300	<t and t<=	400	){	inp[i]=I0* ( (	0.933840 	 +(t-300)*	0.000000 	) *  exp( -( (i-(	96.42966	 +(t-300)*	-0.000223 	))*(i-(	96.42966	 +(t-300)*	-0.000223 	))) / (2*(	39.60195	 +(t-300)*	-0.000065 	 )*(	39.60195	 +(t-300)*	-0.000065 	) ))); }
  	if(	400	<t and t<=	500	){	inp[i]=I0* ( (	0.929461 	 +(t-400)*	-0.000002 	) *  exp( -( (i-(	94.19835	 +(t-400)*	0.000164 	))*(i-(	94.19835	 +(t-400)*	0.000164 	))) / (2*(	38.95008	 +(t-400)*	-0.000231 	 )*(	38.95008	 +(t-400)*	-0.000231 	) ))); }
  	if(	500	<t and t<=	600	){	inp[i]=I0* ( (	0.912863 	 +(t-500)*	-0.000002 	) *  exp( -( (i-(	95.8429	 +(t-500)*	-0.000258 	))*(i-(	95.8429	 +(t-500)*	-0.000258 	))) / (2*(	36.64446	 +(t-500)*	0.000586 	 )*(	36.64446	 +(t-500)*	0.000586 	) ))); }
  	if(	600	<t and t<=	700	){	inp[i]=I0* ( (	0.890503 	 +(t-600)*	-0.000001 	) *  exp( -( (i-(	93.26461	 +(t-600)*	-0.000203 	))*(i-(	93.26461	 +(t-600)*	-0.000203 	))) / (2*(	42.50511	 +(t-600)*	0.000465 	 )*(	42.50511	 +(t-600)*	0.000465 	) ))); }
  	if(	700	<t and t<=	800	){	inp[i]=I0* ( (	0.877132 	 +(t-700)*	-0.000002 	) *  exp( -( (i-(	91.42455	 +(t-700)*	-0.000184 	))*(i-(	91.42455	 +(t-700)*	-0.000184 	))) / (2*(	46.254	 +(t-700)*	0.000375 	 )*(	46.254	 +(t-700)*	0.000375 	) ))); }
  	if(	800	<t and t<=	900	){	inp[i]=I0* ( (	0.866067 	 +(t-800)*	-0.000001 	) *  exp( -( (i-(	89.15006	 +(t-800)*	-0.000227 	))*(i-(	89.15006	 +(t-800)*	-0.000227 	))) / (2*(	44.75368	 +(t-800)*	-0.000150 	 )*(	44.75368	 +(t-800)*	-0.000150 	) ))); }
  	if(	900	<t and t<=	1000	){	inp[i]=I0* ( (	0.860535 	 +(t-900)*	-0.000001 	) *  exp( -( (i-(	88.34304	 +(t-900)*	-0.000081 	))*(i-(	88.34304	 +(t-900)*	-0.000081 	))) / (2*(	43.55638	 +(t-900)*	-0.000120 	 )*(	43.55638	 +(t-900)*	-0.000120 	) ))); }
  	if(	1000	<t and t<=	1100	){	inp[i]=I0* ( (	0.849470 	 +(t-1000)*	0.000000 	) *  exp( -( (i-(	87.79559	 +(t-1000)*	-0.000055 	))*(i-(	87.79559	 +(t-1000)*	-0.000055 	))) / (2*(	43.604	 +(t-1000)*	0.000005 	 )*(	43.604	 +(t-1000)*	0.000005 	) ))); }
  	if(	1100	<t and t<=	1200	){	inp[i]=I0* ( (	0.844629 	 +(t-1100)*	0.000000 	) *  exp( -( (i-(	85.59566	 +(t-1100)*	-0.000220 	))*(i-(	85.59566	 +(t-1100)*	-0.000220 	))) / (2*(	38.81664	 +(t-1100)*	-0.000479 	 )*(	38.81664	 +(t-1100)*	-0.000479 	) ))); }
  	if(	1200	<t and t<=	1300	){	inp[i]=I0* ( (	0.841402 	 +(t-1200)*	-0.000001 	) *  exp( -( (i-(	88.011	 +(t-1200)*	0.000242 	))*(i-(	88.011	 +(t-1200)*	0.000242 	))) / (2*(	47.64612	 +(t-1200)*	0.000883 	 )*(	47.64612	 +(t-1200)*	0.000883 	) ))); }
  	if(	1300	<t and t<=	1400	){	inp[i]=I0* ( (	0.835178 	 +(t-1300)*	0.000000 	) *  exp( -( (i-(	88.04742	 +(t-1300)*	0.000004 	))*(i-(	88.04742	 +(t-1300)*	0.000004 	))) / (2*(	51.14102	 +(t-1300)*	0.000349 	 )*(	51.14102	 +(t-1300)*	0.000349 	) ))); }
  	if(	1400	<t and t<=	1500	){	inp[i]=I0* ( (	0.837944 	 +(t-1400)*	0.000002 	) *  exp( -( (i-(	89.2045	 +(t-1400)*	0.000116 	))*(i-(	89.2045	 +(t-1400)*	0.000116 	))) / (2*(	48.55413	 +(t-1400)*	-0.000259 	 )*(	48.55413	 +(t-1400)*	-0.000259 	) ))); }
  	if(	1500	<t and t<=	1600	){	inp[i]=I0* ( (	0.853389 	 +(t-1500)*	0.000002 	) *  exp( -( (i-(	90.90585	 +(t-1500)*	0.000170 	))*(i-(	90.90585	 +(t-1500)*	0.000170 	))) / (2*(	52.54611	 +(t-1500)*	0.000399 	 )*(	52.54611	 +(t-1500)*	0.000399 	) ))); }
  	if(	1600	<t and t<=	1700	){	inp[i]=I0* ( (	0.868834 	 +(t-1600)*	0.000004 	) *  exp( -( (i-(	94.66218	 +(t-1600)*	0.000376 	))*(i-(	94.66218	 +(t-1600)*	0.000376 	))) / (2*(	51.94713	 +(t-1600)*	-0.000060 	 )*(	51.94713	 +(t-1600)*	-0.000060 	) ))); }
